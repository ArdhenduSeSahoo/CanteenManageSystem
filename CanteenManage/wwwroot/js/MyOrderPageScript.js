...
$(function () {
    ...
    var qrcode = new QRCode(document.getElementById("sldkqrsldkf"), {
        width: 250,
        height: 250
    });

    $('#viewQrCodeModal').on('show.bs.modal', function (event) {
        // Code to execute after the modal is fully shown
        const button = event.relatedTarget
        // Extract info from data-bs-* attributes data-bs-rating
        var qrcodetext = button.getAttribute('data-bs-codetext');
        //
        //$(this).find('input:first').focus();
        qrcode.clear();
        qrcode.makeCode(qrcodetext);
        $(this).find('#sldkqrsldkf').attr("title", '');
    });

    $('#viewQrCodeModal').on('hidden.bs.modal', function () {
        // Code to execute after the modal is fully hidden
        //$(this).find('#sldkqrsldkf').html("");
        //debugger;
        qrcode.clear();
        qrcode.makeCode("clear");
    });
});
